...
function calculate() {
  ...
  if (custom) {
    ...
    totalPerPerson.textContent = Number.parseFloat(
      (totalTip + bill) / people
    ).toFixed(2);
  } else {
    let tip = document.querySelector('input[name = "tip"]:checked').value;
    tipPerPerson.textContent = Number.parseFloat(
      (tip * bill - bill) / people
    ).toFixed(2);
    totalPerPerson.textContent = Number.parseFloat(tip * bill / people).toFixed(
      2
    );
  }
}
